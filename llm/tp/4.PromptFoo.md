...
`PromptFoo` permet d'automatiser les requêtes sur une API `chat_completions` et journalise les résultats. Le prompt est très dynamique, tout est enregistré dans des fichiers, ce qui permet une excellente traçabilité. Une interface web est disponible pour analyser les résultats de campagnes. 

*Je déconseille d'utiliser l'interface pour configurer les tâches, préférer travailler sur les fichiers de configuration. L'interface ne doit servir que de visualisation.*
